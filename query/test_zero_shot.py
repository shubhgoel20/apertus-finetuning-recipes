...
MAX_NEW_TOKENS = 512         # Reduce from 512 (baseline gives short answers)
...
print(f"--- Fast MMLU Evaluation for {model_path} ---")
...
# 3. Helper: Format MMLU Example
def format_mmlu_prompt(example):
    """
    MMLU comes as:
    {'question': '...', 'choices': ['opt1', 'opt2', 'opt3', 'opt4'], 'answer': 0}
    We format it to match the training format exactly.
    """
...
for task in TASKS:
    ...
    try:
        ds = load_dataset(dataset_base_path, task, split="test", trust_remote_code=True)
...
# 6. Final Report
print("\n" + "="*60, flush=True)
print("BASELINE MODEL - MMLU MEDICAL RESULTS", flush=True)
print("="*60, flush=True)
avg_acc = 0
